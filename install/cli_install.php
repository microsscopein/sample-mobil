...
//
// Command line tool for installing sample
// Author: OFA <bikopar@gmail.com>
//
...
//                               --http_server http://localhost/sample
//
...
function check_requirements() {
	$error = null;
	if (phpversion() < '5.0') {
		$error = 'Warning: You need to use PHP5 or above for sample to work!';
	}
	...
	if (ini_get('session.auto_start')) {
		$error = 'Warning: sample will not work with session.auto_start enabled!';
	}

	if (!extension_loaded('mysqli')) {
		$error = 'Warning: MySQLi extension needs to be loaded for sample to work!';
	}

	if (!extension_loaded('gd')) {
		$error = 'Warning: GD extension needs to be loaded for sample to work!';
	}

	if (!extension_loaded('curl')) {
		$error = 'Warning: CURL extension needs to be loaded for sample to work!';
	}

	if (!function_exists('mcrypt_encrypt')) {
		$error = 'Warning: mCrypt extension needs to be loaded for sample to work!';
	}

	if (!extension_loaded('zlib')) {
		$error = 'Warning: ZLIB extension needs to be loaded for sample to work!';
	}
...
case "install":
	try {
		...
		install($options);
		echo "SUCCESS! sample successfully installed on your server\n";
